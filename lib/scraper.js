...
function string_to_slug(str) {
  ...
  var from = "Ñåàáãäâèéëêìíïîòóöôùúüûñç·/_,:;";
  var to = "Naaaaaaeeeeiiiioooouuuunc------";
...
function getBasicInformation(html , callback) {
	try {
		...
		contribuyente.direccion_referencia = 'p'+string_to_slug(table.eq(6).children().eq(1).text().split("-").map(function (splited) {
			return splited.trim();
		}).join("-"));
